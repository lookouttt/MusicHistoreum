...
**Apple Music playlist export (`src/features/appleMusicPlaylist/`):** lets a user turn a selection of chart rows into an Apple Music library playlist. `ChartCard` renders `AppleMusicPlaylistToolbar` only for the main chart page's Song charts (`bIncludeNav && chartType === 'Song'`) — album charts and the embedded/secondary chart views don't get it. Flow: `musicKitLoader` lazy-loads the MusicKit JS SDK from Apple's CDN → `musicKitAuth` fetches a developer token via `fetchAppleMusicDeveloperToken` (calling the server's `/apple-music/developer-token`) and prompts the user to authorize their Apple Music account → `CreatePlaylistModal` collects a playlist name (or an existing playlist to add to) and calls `createAppleMusicPlaylist`. This feature went through extensive real-world tuning against a ~6,500-song playlist to eliminate false non-matches and duplicate tracks; the pieces below exist because of specific failure modes hit during that testing, not speculative design.

**Matching (`songMatcher.js`):** for each song, `findLibraryMatch` checks the user's own Apple Music library first (`/v1/me/library/search`) before ever querying the catalog — a hit is added directly with no catalog search, since Apple Music additions, matched uploads, and plain uploads all show up identically in library search and any of them means the user already owns it. Only a library miss falls through to `searchCatalog`. Search terms go through `searchTitle`, which strips trailing parenthetical/bracketed annotations (soundtrack attributions like `(From "Top Gun")`, "(2019 Remaster)", "(Single Version)", "(feat. X)", etc.) since Billboard's title often carries one Apple's doesn't — both for the query itself and, via `cacheKey`, for dedup comparisons. Artist comparison (`artistsLooselyMatch`/`namesLooselyMatch`) handles accent stripping (Unicode NFD decomposition — deleting an accent outright instead of transliterating it breaks a mid-word case like "Céline" differently than an end-of-word case like "Beyoncé"), numeral/word equivalence ("4" ↔ "four"), an `&`/"and" equivalence, a small hardcoded alias table for renames no algorithm can infer (Ke$ha→Kesha, Dixie Chicks→The Chicks), and token-subsequence matching for inserted nicknames ("Missy 'Misdemeanor' Elliott"→"Missy Elliott"). Every search result is cached across the whole run by an in-flight *promise* (not just the resolved value) keyed by normalized title+artist, so two chart rows for the same recurring song share one search instead of two concurrent workers racing independent ones and potentially resolving to two different catalog editions. Rate-limited/empty-result misses get one or two automatic retries at reduced concurrency once the initial request burst clears. When a search finds candidates but none pass the artist check, those candidates — plus a second artist-only search, to recover cases where a cover/medley ranks below a more popular original sharing the same title words — are surfaced in the UI for manual review instead of just failing outright.

**Duplicate prevention (`createAppleMusicPlaylist.js`):** when adding to an existing playlist, its current tracks are fetched first (paginated 100 at a time, with per-page retry-on-failure, and `cache: 'no-store'` since the same URL can be re-requested across runs in one page session) and compared against the input selection *before* any searching happens — a song already there is skipped entirely rather than independently re-resolved, since a fresh search can land on a different-but-equally-valid catalog edition (single vs. album version, remaster vs. original) than whatever's already on the playlist, which a comparison done only *after* searching can't reliably catch. Comparison uses both an id-based identity (catalog id, falling back to a library-song id when there's no catalog counterpart) and a normalized `title::artist` text key (`cacheKey`, exported from `songMatcher.js`) so a mismatch in one signal alone can't let a duplicate through — `cacheKey`'s artist half goes through `primaryArtist` (drops "Featuring X"/"feat. X" collaborator credits) and `canonicalArtist` (the same alias/numeral/squashing normalization used for matching), which turned out to be the single biggest source of false "not a duplicate" results, since Billboard and Apple often word the same collaborator credit differently. A slower loose-comparison fallback (reusing the matcher's full leniency, not just the fast key) catches songs only resolvable through manual review, whose Apple-side wording doesn't line up with Billboard's closely enough for the fast check — including comparing an unmatched song's own candidate list against the playlist directly, so something already there resolves silently instead of prompting for manual review at all. The same known-identity data is exposed to `UnmatchedSongItem`'s manual "add this candidate" picker so a manual pick that turns out to already be on the playlist is recognized instead of re-added. `createAppleMusicPlaylist.js` currently still has a few `console.log('[AppleMusicPlaylist] ...')` diagnostic calls (existing-track key dump, per-song matched/duplicate outcome, a selection-vs-playlist reconciliation diff) left in from that tuning process — safe to remove once the matching/dedup logic has proven stable for a while, but useful for tracing a future report of the same symptoms.

**Result persistence:** the completed run's summary is saved to `sessionStorage` (`CreatePlaylistModal`'s `LAST_RESULT_KEY`) so it survives the modal closing unexpectedly (e.g. the tab losing focus for long enough); a "View results from your last completed run" link on the idle screen restores it. The modal uses `backdrop="static"` so it can only be closed by an explicit button click, not a stray click/interaction.
